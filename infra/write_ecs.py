path = '/Users/suavemax27/scoutcloud/scoutcloud.dev/infra/ecs.tf'
content = '''resource "aws_iam_role" "ecs_task" {
...
resource "aws_iam_role_policy" "ecs_task_dynamodb" {
  ...
  policy = jsonencode({
    Version = "2012-10-17"
    Statement = [{ Effect = "Allow", Action = ["dynamodb:PutItem", "dynamodb:GetItem", "dynamodb:UpdateItem"], Resource = aws_dynamodb_table.live_scores.arn }]
  })
}
...
resource "aws_security_group" "vpce" {
  ...
  ingress { from_port = 443, to_port = 443, protocol = "tcp", security_groups = [aws_security_group.ecs_tasks.id] }
  egress { from_port = 0, to_port = 0, protocol = "-1", cidr_blocks = ["0.0.0.0/0"] }
...
resource "aws_vpc_endpoint" "s3" {
  vpc_id            = data.aws_vpc.default.id
  service_name      = "com.amazonaws.${local.vpce_region}.s3"
...
resource "aws_vpc_endpoint" "ecr_api" {
  vpc_id              = data.aws_vpc.default.id
  service_name        = "com.amazonaws.${local.vpce_region}.ecr.api"
...
resource "aws_vpc_endpoint" "ecr_dkr" {
  vpc_id              = data.aws_vpc.default.id
  service_name        = "com.amazonaws.${local.vpce_region}.ecr.dkr"
  vpc_endpoint_type   = "Interface"
  subnet_ids          = data.aws_subnets.default.ids
  security_group_ids  = [aws_security_group.vpce.id]
  private_dns_enabled = true
...
resource "aws_vpc_endpoint" "logs" {
  vpc_id              = data.aws_vpc.default.id
  service_name        = "com.amazonaws.${local.vpce_region}.logs"
  vpc_endpoint_type   = "Interface"
  subnet_ids          = data.aws_subnets.default.ids
  security_group_ids  = [aws_security_group.vpce.id]
  private_dns_enabled = true
...
resource "aws_ecs_service" "score_fetcher" {
  namepath = 
'/Users/suavemax27/scoutcloud/scoutcloud.dev/infra/ecs.tf'
lines = []
lines.append('resource "aws_iam_role" "ecs_task" {\n')
lines.append('  name = "scoutcloud-ecs-task-role"\n')
lines.append('  assume_role_policy = jsonencode({\n')
lines.append('    Version = "2012-10-17"\n')
lines.append('    Statement = [{\n')
lines.append('      Effect    = "Allow"\n')
lines.append('      Principal = { Service = "ecs-tasks.amazonaws.com" }\n')
